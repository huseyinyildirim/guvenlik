...
public partial class projeler_zimmet_ekle : System.Web.UI.Page
{
...
{
string SQL = "SELECT id,(SELECT urun FROM urun USE INDEX (id) WHERE id=urun_id) as urun,(SELECT kod FROM urun USE INDEX (id) WHERE id=urun_id) as kod,durum,adet,tamam FROM proje_urun USE INDEX (proje_id) WHERE proje_id='" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"]) + "'";
...
{
case "0":
e.Row.Cells[6].Text = "<a href=\"?id=" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"]) + "&amp;sil=" + ID + "\" class=\"sil\" id=\"proje_urun_sil_" + ID + "\"><img src=\"images/ikon/sil.png\" class=\"ikon\" alt=\"\"/></a>";
break;
...
{
string SQL1 = "SELECT urun_id,durum,adet FROM proje_urun USE INDEX (proje_id,tamam) WHERE proje_id='" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"]) + "' and tamam='0'";
...
string SQL2 = "INSERT INTO depo (bolge_id,firma_id,proje_id,urun_id,durum,adet,tip,admin_id) values (@bolge_id,@firma_id,@proje_id,@urun_id,@durum,@adet,@tip,@admin_id)";
...
string SQL3 = "UPDATE proje_urun SET tamam='1' WHERE proje_id='" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"]) + "' and tamam='0'";
Class.Fonksiyonlar.MySQL.ExecuteNonQuery(SQL3);
}

}

ClientScript.RegisterClientScriptBlock(this.GetType(), Class.Degiskenler.SiteAdi, Class.Fonksiyonlar.JavaScript.Yonlendir("projeler-personel-ekle.aspx?id=" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"])), true);
}
...
string SQL2 = "SELECT COUNT(id) FROM proje_urun USE INDEX (proje_id,urun_id,durum) WHERE proje_id ='" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["id"]) + "' and urun_id ='" + urun_id + "' and durum ='" + urun_durumu_son + "'";
string sonuc = Class.Fonksiyonlar.MySQL.ExecuteScalar(SQL2);

if (sonuc == "0")
{
string SQL3 = "INSERT INTO proje_urun (bolge_id,firma_id,proje_id,urun_id,durum,adet,admin_id) values (@bolge_id,@firma_id,@proje_id,@urun_id,@durum,@adet,@admin_id)";
...
if (Class.Degiskenler.OturumVarmi == 1)
{
string SQL = "DELETE FROM proje_urun WHERE id='" + Class.Fonksiyonlar.SQLTemizle(Request.QueryString["sil"]) + "'";
