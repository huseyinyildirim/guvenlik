...
public partial class ana_depo_islemler : System.Web.UI.Page
{
...
string SQL = "SELECT urun_id,(SELECT urun FROM urun USE INDEX (id) WHERE id=depo.urun_id) as urun,(SELECT kod FROM urun USE INDEX (id) WHERE id=depo.urun_id) as kod,(SELECT adet FROM depo_urunler_toplamlar WHERE urun_id=depo.urun_id and durum=depo.durum GROUP BY urun_id,durum) as adet,(SELECT SUM(adet) FROM depo_urunler_say WHERE urun_id=depo.urun_id and durum=depo.durum) as islem_adet,durum FROM depo USE INDEX (tip,urun_id,durum) WHERE tip='0' GROUP BY urun_id,durum LIMIT " + limit + "," + Class.Degiskenler.SayfalamaSayisi + "";
...
{
string SQL = "SELECT COUNT(id) FROM depo_urunler";
...
nc1.Text = "<h3><img src=\"images/ikon/folder-open.png\" class=\"ikon\" alt=\"\"/> Depodaki İşlemler (Ürün Bazlı)</h3>";
...
e.Row.Cells[6].Text = Class.Fonksiyonlar.SayiFormatla(e.Row.Cells[6].Text);

e.Row.Cells[7].Text = "<a href=\"ana-depo-urun-detay.aspx?id=" + ID + "&amp;durum=" + drm + "\" id=\"urun_detay_" + ID + "\"><img src=\"images/ikon/notes.png\" class=\"ikon\" alt=\"\"/></a>";
}
